...
export function Sidebar({ instant = false }: SidebarProps) {
  ...
  return (
    <motion.aside initial={instant ? false : { x: -100 }} animate={{ x: 0 }} transition={{ duration: instant ? 0 : 1, ease: [0.22, 1, 0.36, 1], delay: instant ? 0 : 0.2 }} className="hidden md:flex fixed left-0 top-0 h-full z-40 flex-col py-10 bg-background/90 backdrop-blur-xl border-r border-primary/10 group w-20 hover:w-64 transition-all duration-500 overflow-hidden">
      <div className="flex flex-col items-center group-hover:items-start px-6 gap-2 mb-12 mt-16">
        <div className="w-10 h-10 rounded-full border border-primary/30 flex items-center justify-center bg-surface-container-high overflow-hidden shadow-[0_0_15px_rgba(216,196,145,0.2)] group-hover:shadow-[0_0_20px_rgba(216,196,145,0.4)] transition-shadow duration-500 shrink-0">
      ...
      <nav className="flex-1 flex flex-col gap-8 px-6">
        ...
        <SidebarLink to="/projects" icon="terminal" label="Projects" color="secondary" />
        <SidebarLink to="/about" icon="hotel_class" label="About" color="secondary" />
        <SidebarLink to="/contact" icon="mark_email_read" label="Contact" color="secondary" />
        ...
        {user?.isAdmin && <SidebarExternalLink href="/admin/projects" icon="folder_managed" label="Projects Admin" />}
      </nav>
      <div className="px-4 space-y-4">
        {authenticated && user ? (
          ...
          <button onClick={() => navigate('/login')} className="w-full py-4 bg-primary-container/50 hover:bg-primary-container text-primary font-label text-[10px] uppercase tracking-[0.2em] rounded-md opacity-0 group-hover:opacity-100 transition-all duration-500 shadow-[inset_0_0_12px_rgba(216,196,145,0.1)] hover:shadow-[inset_0_0_20px_rgba(216,196,145,0.3)] border border-primary/20 whitespace-nowrap" type="button">Enter the Void</button>
        )}
      </div>
    </motion.aside>
  );
    ...
    <NavLink to={to} className={({ isActive }) => `flex items-center gap-4 py-2 transition-all duration-300 relative ${isActive ? colorClass : inactiveClass}`}>
      {({ isActive }) => (
        <>
          ...
          <span className={`material-symbols-outlined relative z-10 ${isActive ? `drop-shadow-[0_0_8px_rgba(${color === 'primary' ? '216,196,145' : '225,193,152'},0.8)]` : ''}`}>{icon}</span>
...
function SidebarExternalLink({ href, icon, label }: { href: string; icon: string; label: string }) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noreferrer"
      className="flex items-center gap-4 py-2 transition-all duration-300 text-secondary/40 hover:text-secondary"
    >
      <span className="material-symbols-outlined">{icon}</span>
      <span className="hidden group-hover:block font-label tracking-widest text-xs uppercase whitespace-nowrap">{label}</span>
    </a>
  );
}
